...
class PeopleHelperTest < ActionView::TestCase
  ...
  Person.dens.each_key do |den|
    test "the badge for #{den} shows its label and unregistered status" do
      assert_dom_equal %(<span class="#{BADGE_CLASSES}">#{I18n.t("people.dens.#{den}")} · #{I18n.t("people.unregistered")}</span>),
                       den_badge(Person.new(den: den))
    end
  end

  test "the badge uses the display label, not the raw den" do
    assert_includes den_badge(Person.new(den: "aol")), ">AOL · Unregistered<"
  ...
  test "an adult with no regular-den family members gets a plain badge" do
    solo_adult = families(:admin).people.create!(first_name: "Ada", last_name: "Admin", den: :adult, scouting_america_id: "SA-100003")
